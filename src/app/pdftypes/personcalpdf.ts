...
export class PersonCalPDF {
    ...
    public async pdfIt() {
        ...
        await this.getTheData(doc);

    }

    private async doIt(doc: any) {
        ...
        for (var taskObj of this.taskObjs) {
            ...
            var isPerson = false;
            if (taskObj.task.empl_key == this.scs.getPerson().key) {
                isPerson = true;
            }
        ...
        console.log("this.medTasks().length=" + this.medTasks.length);
    ...
    doHeader1(doc: any) {
        ...
        var personName: string = this.scs.getPersonName(this.scs.getPerson().key);
    ...
    filterPerson(person: Person): boolean {
        if (person.last_name == "Burman" ||
            person.last_name == "Lehnert" ||
            person.last_name == "Verrastro" ||
            person.last_name == "Edelstein" ||
            person.last_name == "Hagenbuch" ||
            person.last_name == "Braye" ||
            person.last_name == "Marchetti" ||
            person.last_name == "Newcomer" ||
            person.last_name == "Smith" ||
            person.last_name == "DuBoice" ||
            person.last_name == "Kuna" ||
            person.last_name == "Gelso" ||
            person.last_name == "Haigh" ||
    ...
    async getTheData(doc: any) {
        var taskCollection = this.afs.collection<Task>('tasks', ref => ref.where('empl_key', '==', this.scs.getPerson().key));
        taskCollection.ref.get().then(async (querySnapshot) => {
            ...
            for (var task of this.tasks) {
                if (task.empl_key != this.scs.getPerson().key) continue;
